    ...
    <div className="min-h-screen relative overflow-hidden flex flex-col items-center justify-center p-6 text-center">
      ...
      >
        ...
        >
          ...
          The Future of Local Video is Here
        </motion.div>

        <h1 className="text-7xl lg:text-9xl font-black leading-[0.95] tracking-tighter mb-10">
          Studio Power. <br />
          <span className="text-zinc-400 dark:text-zinc-600">Zero Uploads.</span>
        </h1>
        
        <p className="text-xl lg:text-2xl text-zinc-500 dark:text-zinc-400 font-medium mb-16 max-w-2xl mx-auto leading-relaxed">
          The ultimate desktop studio for creators. Convert, crop, and transcribe with 100% privacy using local WASM and AI engines.
        </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
          {[
            { title: "Privacy First", desc: "No data ever leaves your machine. 100% secure.", icon: Shield, color: "text-pink-500" },
            { title: "WASM Speed", desc: "High-performance processing directly in your sandbox.", icon: Zap, color: "text-blue-500" },
            { title: "Local AI", desc: "Whisper AI models running locally on your hardware.", icon: CheckCircle2, color: "text-green-500" },
          ].map((card, i) => (
      ...
      <footer className="absolute bottom-10 left-0 right-0 opacity-20">
        <div className="text-[10px] font-black uppercase tracking-[0.4em]">
          Local-First Architecture • Next.js 15 • WASM Engine
        </div>
      </footer>
    </div>
  );
